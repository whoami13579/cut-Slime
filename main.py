    ...
    if y1 < height and y2 > 0 and x1 < width and x2 > 0:
        ...
        for c in range(0, 3):
            canvas[y1:y2, x1:x2, c] = (
                alpha_s * knife_img[img_y1:img_y2, img_x1:img_x2, c]
...
def draw_pause(x, y):
    ...
    if y1 < height and y2 > 0 and x1 < width and x2 > 0:
        ...
        for c in range(0, 3):
            canvas[:100, WIDTH - 100 :, c] = (
                alpha_s * pause_img[:, :, c] + alpha_l * canvas[:100, WIDTH - 100 :, c]
            )
...
def draw_restart(x, y):
    ...
    if y1 < height and y2 > 0 and x1 < width and x2 > 0:
        ...
        for c in range(0, 3):
            canvas[y : y + 100, x : x + 200, c] = (
                alpha_s * restart_img[:, :, c]
...
play_img = cv2.imread("background_play_2.png", cv2.IMREAD_UNCHANGED)
if heart_img is None:
    print("Error: Unable to load heart image.")
    sys.exit()

knife_img = cv2.imread("sword.png", cv2.IMREAD_UNCHANGED)
if knife_img is None:
...
class Fruit:
    def __init__(self, img, value, hit_img=None, life=1):
        ...
        if self.spawn_side == "bottom":
            self.x = random.randint(self.center_x + 200, width - self.center_x - 200)
            self.y = height + self.center_y
            self.angle_direction = random.uniform(
                5 * math.pi / 6, math.pi / 6
            )  # 150 to 30 degrees
        elif self.spawn_side == "left":
            self.x = -self.center_x
            self.y = random.randint(self.center_y + 200, height - self.center_y - 200)
            self.angle_direction = random.uniform(
                -math.pi / 4, math.pi / 4
            )  # -45 to 45 degrees
    ...
    def draw(self, canvas):
        ...
        # 將物件的位置和尺寸轉換為整數
        y1, y2 = max(0, int(self.y - new_center_y)), min(
            height, int(self.y + new_h - new_center_y)
        )
        x1, x2 = max(0, int(self.x - new_center_x)), min(
            width, int(self.x + new_w - new_center_x)
        )
        ...
        if y1 < height and y2 > 0 and x1 < width and x2 > 0:
            ...
            for c in range(0, 3):
                canvas[y1:y2, x1:x2, c] = (
                    alpha_s * rotated_img[img_y1:img_y2, img_x1:img_x2, c]
                    + alpha_l * canvas[y1:y2, x1:x2, c]
                )
    ...
    def draw(self, canvas):
        if self.current_time < self.duration:
            ...
            scaled_img = cv2.resize(
                self.img, (int(w * self.scale), int(h * self.scale))
            )
            scaled_h, scaled_w, _ = scaled_img.shape

            y1, y2 = max(0, int(self.y - scaled_h // 2)), min(
                height, int(self.y + scaled_h // 2)
            )
            x1, x2 = max(0, int(self.x - scaled_w // 2)), min(
                width, int(self.x + scaled_w // 2)
            )
            if y1 < height and y2 > 0 and x1 < width and x2 > 0:
                ...
                img_x2 = img_x1 + (x2 - x1)
                alpha_s = (
                    scaled_img[img_y1:img_y2, img_x1:img_x2, 3] / 255.0 * self.alpha
                )
                alpha_l = 1.0 - alpha_s
                for c in range(0, 3):
                    canvas[y1:y2, x1:x2, c] = (
                        alpha_s * scaled_img[img_y1:img_y2, img_x1:img_x2, c]
                        + alpha_l * canvas[y1:y2, x1:x2, c]
                    )
            self.scale += 0.1
        ...
        while True:
            ...
            while life > 0:  # 生命大於0時遊戲繼續
                ...
                if current_time - last_spawn_time > spawn_interval:
                    obj_type = random.choice(
                        [
                            ...
                            Fruit(bomb_img, -1),
                            Fruit(
                                red_slime_img, 1, hit_img=red_slime_hit_img, life=2
                            ),  # 添加紅史萊姆
                        ]
                    )
                ...
                for obj in objects:
                    obj.move()
                    if (
                        obj.y - obj.center_y > height
                        or obj.x - obj.center_x > width
                        or obj.y + obj.center_y < 0
                        or obj.x + obj.center_x < 0
                    ):
                ...
                # 移除已经结束的爆炸效果
                explosions = [
                    exp for exp in explosions if exp.current_time < exp.duration
                ]
                ...
                for i in range(life):
                    ...
                    for c in range(0, 3):
                        canvas[
                            heart_y : heart_y + heart_img.shape[0],
                            heart_x : heart_x + heart_img.shape[1],
                            c,
                        ] = (
                            alpha_s * heart_img[:, :, c]
                            + alpha_l
                ...
                if history_scores:
                    cv2.putText(
                        canvas,
                        f"History Score: {history_scores[0]}",
                        (width - 300, 30),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        1,
                        (255, 255, 255),
            ...
            if life <= 0:
                ...
                history_scores = sorted(history_scores, reverse=True)[
                    :5
                ]  # 保留最高的五個分數
                ...
                while True:
                    ...
                    cv2.putText(
                        canvas,
                        "Press 'W' to Retart or 'Q' to Quit",
                        (width // 2 - 275, height // 2 + 200),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        1,
                        (0, 0, 0),
                        2,
